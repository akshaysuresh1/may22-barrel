...
# CREATE A LIST OF SAMPLES FOR EACH STATE (SORTED IN ALPHABETICAL ORDER)
states_all <- vector( "list", length = n_states )
for( i in 1:length( state_names ) ){
  states_all[[i]] <- subset( rice_yield_df, State.Name == state_names[i] )
}
...
corr_matrix <- matrix( rep( NA, n_states*n_covariates ), nrow = n_covariates, ncol = n_states )
pvalue_matrix <- matrix( rep( NA, n_states*n_covariates ), nrow = n_covariates, ncol = n_states )
for( i in 1:length( state_names ) ){
  for( j in 1:n_covariates ){
    corr_matrix[j,i] <- cor( states_all[[i]][,6+j], states_all[[i]]$RICE.YIELD..kg.per.ha., use = 'na.or.complete', method = corr_method )
    pvalue_matrix[j,i] <- cor.test( states_all[[i]][,6+j], states_all[[i]]$RICE.YIELD..kg.per.ha., conf.level = conf_level, method = corr_method )$p.value
  }
}
colnames( corr_matrix ) <- state_names
rownames( corr_matrix ) <- colnames( rice_yield_df )[7:(6+n_covariates)]
colnames( pvalue_matrix ) <- state_names
rownames( pvalue_matrix ) <- colnames( rice_yield_df )[7:(6+n_covariates)]



# CLUSTER COVARIATES BY THEIR CORRELATIONS WITH YIELD DATA BY STATE
# COMPUTE DISTANCE MATRIX FOR THE COVARIATES BY THEIR CORRELATIONS TO THE RICE YIELD DATA ON DIFFERENT STATES
...
# CLUSTER STATES BY THEIR CORRELATIONS BETWEEN THEIR YIELD VALUES AND THE COVARIATES
# COMPUTE DISTANCE MATRIX FOR THE STATES BY THEIR CORRELATIONS TO THE COVARIATES
distance_states <- dist( t( corr_matrix ), method = "euclidian" )
# CLUSTER THE STATES BY HIERARCHICAL CLUSTERING
cluster_states <- hclust( distance_states, method = "ward.D2" )
...
plot_heatmap <- function( input_matrix, color_list, color_scale_breaks, matrix_name ){
  ...
  heatmap.2( input_matrix,
             ...
             Rowv = as.dendrogram( cluster_states ),
...
pdf( file = paste0( outdir, "/corr_matrix_heatmap_states_", corr_method, ".pdf" ), width = 8.5, height = 11 )
...
pdf( file = paste0( outdir, "/pvalue_matrix_heatmap_states_", corr_method, ".pdf" ), width = 8.5, height = 11 )
...
corr_matrix_signif <- matrix( rep( NA, n_states*n_covariates ), nrow = n_covariates, ncol = n_states )
...
pvalue_matrix_fdr_corrected_signif <- matrix( rep( NA, n_states*n_covariates ), nrow = n_covariates, ncol = n_states )
for( j in 1:dim( pvalue_matrix_fdr_corrected )[1] ){
  for( i in 1:dim( pvalue_matrix_fdr_corrected )[2] ){
    if( pvalue_matrix_fdr_corrected[j,i] < 0.05 ){
...
pdf( file = paste0( outdir, "/corr_matrix_signif_heatmap_states_", corr_method, ".pdf" ), width = 8.5, height = 11 )
...
pdf( file = paste0( outdir, "/pvalue_matrix_fdr_corrected_signif_heatmap_states_", corr_method, ".pdf" ), width = 8.5, height = 11 )
